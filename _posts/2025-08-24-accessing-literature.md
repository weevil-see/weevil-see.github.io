...
**For PDF, its a bit more complicated.** The PDF format itself is free and so widely used that it will certainly be accessible in the future. But PDF can embed other files, such as images, in their original format. The PDF/A (A for archive) standard tries to include as much as possible within the file, so that it doesn't rely on external resources (such as fonts), which may not be available in the future. Storing PDF in PDF/A can increase the size of the file.
